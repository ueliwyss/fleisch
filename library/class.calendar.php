...
var $months_en=array('January','February','March','April','May','June','July','August','September','October','November','December');
var $days_en=array('Su','Mo','Tu','We','Th','Fr','Sa');

var $months_fr=array('Janvier','Février','Mars','Avril','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','Décembre');
var $days_fr=array('Di','Lu','Ma','Me','Je','Ve','Sa');

var $months_it=array('Gennaio','Febbraio','Marzo','Aprile','Maggio','Giugno','Luglio','Agosto','Settembre','Ottobre','Novembre','Dicembre');
var $days_it=array('Do','Lu','Ma','Me','Gi','Ve','Sa');
...
var $cssLegend="legend";
var $cssLegendItem='legendItem';
/*
********************************************************************************
...
********************************************************************************
*/
function calendar($year=false,$month=false,$day=false,$GMTDiff="none"){
$this->timetoday=time();
$this->selectedday=-2;
...
public function setColorArea($dateFrom,$dateTo,$color,$content) {
	global $db;

	$id='area_'.substr($color,1);
	if(!$this->areas[$color]) {
		$this->areas[$color]=$content;
		$this->content['CSS'].='
.'.$id.' {
	text-align: center;
	font-size: 11px;
	...
	$num_days=div::date_diff('d',$dateFrom,$dateTo." 01:00:00")+1;



	$date_arr=explode(".",$dateFrom);
	$timestamp=$this->mkActiveTime(0,0,1,$date_arr[1],$date_arr[0],$date_arr[2]);

	for($i=0;$i<$num_days;$i++) {
		$tmpDate=explode(".",date('d.m.Y',div::date_addDays($timestamp,$i)));
		$eventTime=$this->mkActiveTime(0,0,1,$tmpDate[1],$tmpDate[0],$tmpDate[2]);
...
function setLang($lang='de') {
	switch($lang) {
		case 'en': $this->setMonthNames($this->months_en); $this->setDayNames($this->days_en); break;
		case 'fr': $this->setMonthNames($this->months_fr); $this->setDayNames($this->days_fr); break;
		case 'it': $this->setMonthNames($this->months_it); $this->setDayNames($this->days_it); break;
	}
}
/*
********************************************************************************
...
$this->content['main']=$out;
$this->content['JS']=$this->mkJS();
return $this->content;
}
/*
********************************************************************************
...
function mkLegend() {
	global $db;
	if($this->showLegend) {
		$out.='<tr><td class="'.$this->cssLegend.'" colspan="'.$this->rowCount.'">';
		$out.=$this->legendAddition;
			foreach($this->areas as $color=>$content) {
				$out.='<div id="area_'.substr($color,1).'" class="'.$this->cssLegendItem.'" style="background-color:'.$color.';">'.$content['main'].'</div>';
			}
...
function mkJS() {
	return 'function eventOver(over,object) {
	var legenditem=document.getElementById(object.className);
	if(over) {
		legenditem.style.border="2px solid red";
	} else {
...
$out="<tr>\n";
	for ($x=1; $x<=12; $x++) {
		$this->calendar($curyear,$stmonth,false,$this->GMTDiff);
		$out.="<td valign=\"top\">\n".$this->showMonth()."</td>\n";
		if ($x == $TrMaker && $x < 12) {
			$out.="</tr><tr>";
...
$linkstr=$this->mkUrl($this->actyear,$this->actmonth,$var);
	...
	if ($this->isEvent($var)){
		if ($this->eventUrl){
			$out="<td class=\"".$this->eventID."\" onMouseOver=\"javascript:eventOver(true,this);\" onMouseOut=\"javascript:eventOver(false,this);\"><a href=\"".$this->eventUrl."\">".$var."</a>".$eventContent."</td>";
			$this->eventUrl=false;
		} elseif (!$this->dayLinks) $out="<td class=\"".$this->eventID."\" onMouseOver=\"javascript:eventOver(true,this);\" onMouseOut=\"javascript:eventOver(false,this);\">".$var.$eventContent."</td>";
		else $out="<td class=\"".$this->eventID."\" onMouseOver=\"javascript:eventOver(true,this);\" onMouseOut=\"javascript:eventOver(false,this);\">".$linkstr.$eventContent."</td>";
	...
	if ($this->calEvents){
		...
		foreach($this->calEvents as $eventTime => $eventID){
			if ($eventTime==$checkTime){
				...
				elseif ($eventTime==$todayTime && $this->showToday) $this->eventID=$this->cssPrefixTodayEvent." ".$eventID;
