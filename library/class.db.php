 ...
 *
 * $Id: class.t3lib_db.php,v 1.12.2.1 2005/05/20 07:49:21 mundaun Exp $
 *
...
class MySQLDB {
		...
		// Debug:
	private $debugOutput = TRUE;		// Set "TRUE" if you want database errors outputted.
	public $debug_lastBuiltQuery = '';		// Internally: Set to last built query (not necessarily executed...)
	private $store_lastBuiltQuery = TRUE;	// Set "TRUE" if you want the last built query to be stored in $debug_lastBuiltQuery independent of $this->debugOutput
	 ...
	 */
	function exec_SELECT_mm_query($select,$local_table,$mm_table,$mm_key,$mm_foreign_key,$foreign_table,$whereClause='',$groupBy='',$orderBy='',$limit='')	{
		$mmWhere = $local_table ? $local_table.'.uid='.$mm_table.'.'.$mm_key : '';
		$mmWhere.= ($local_table AND $foreign_table) ? ' AND ' : '';
		$mmWhere.= $foreign_table ? $foreign_table.'.uid='.$mm_table.'.'.$mm_foreign_key : '';
	...
	function admin_get_tables()	{
		global $_CONFIG;
		$whichTables = array();
		$tables_result = mysql_list_tables($_CONFIG['MysqlDB']['database'], $this->link);
		if (!mysql_error())	{
	...
	function debug($func)	{
		...
		if ($error)		{
			if(function_exists(raiseMessage) && false) {
				raiseMessage(MESSAGE_ERROR,"Fehler beim Ausführen des SQL-Statements: ".$error,"1x0001");
			} else {
				echo $this->view_array(array(
           ...
           //echo $this->debug_lastBuiltQuery."<br>"; 
        }
	}
		...
		if (is_array($array_in))	{
			...
			while (list($key,$val)=each($array_in))	{
				...
				if (is_array($array_in[$key]))	{
					$result.=MysqlDB::view_array($array_in[$key]);
				} else {
					$result.= '<font face="Verdana,Arial" size="1" color="red">';
					if(!is_resource($val) AND !is_object($val)) {
						$result.=nl2br(htmlspecialchars($val));
					} else {
						$result.=htmlspecialchars('OBJECT!!');
					}
					$result.='<br /></font>';
				}
	...
	//************************
	//Funktionen von Ueli Wyss
	//************************

	function getFieldInfo($result) {
		$fields=Array();
		$num_fields = mysql_num_fields($result);

		for ($i=0; $i < $num_fields; $i++) {
		    $field["type"] = mysql_field_type($result, $i)=="string"?"varchar":mysql_field_type($result, $i);
		    $field["name"] = mysql_field_name($result, $i);
		    $field["len"] = mysql_field_len($result, $i);
		    $field["flags"] = mysql_field_flags($result, $i);
		    $field["autoIncrement"]=preg_match("/auto_increment/",$field["flags"])?true:false;
		    $field["primaryKey"]=preg_match("/primary_key/",$field["flags"])?true:false;
		    $fields[$field["name"]]=$field;
		}
		return $fields;
	}

	function table2Array($fields,$tableName,$whereClause='',$limitString='',$sortby='') {
		$data=array();

		$result = $this->exec_SELECTquery($fields,$tableName,$whereClause,'',$sortby,$limitString,'');
		$fields = $this->getFieldInfo($result);


		while($row=$this->sql_fetch_row($result)) {
			$datarow=Array();
			$i=0;
			foreach($row as $dataVal) {
				if($fields[$i]["type"]=="datetime") {$dataVal=div::Date_sql2ger($dataVal);}

				$datarow[$fields[$i]["name"]] = $dataVal;
				$i++;
			}
	...
	function insRow($tableName,$data,$dupAllowed=true) {
		if(!$dupAllowed) {
			$res=$this->exec_selectQuery("*",$tableName,'');
			$fields=$this->getFieldInfo($res);
			$i=0;
			foreach($data as $key => $value) {
				if(!div::DB_isDateField($fields,$key)) {
			...
			$res=$this->exec_SELECTquery("*",$tableName,$whereClause);
			$num=$this->sql_num_rows($res);
			if(!$num==0) {
				$stop=true;
			}
		}
		...
		if(!$stop) {
			$res=$this->exec_INSERTquery($tableName,$data);
			$id=$this->sql_insert_id($res);
		}
		return $id;
	}

	function updRow($tableName,$uid,$data) {
		return $this->exec_UPDATEquery($tableName,'uid='.$uid,$data);
	}

	function delRow($tableName,$uid) {
		return $this->exec_DELETEquery($tableName,'uid='.$uid);
	}

	function isDateField($fields,$fieldname) {
	...
	/**
	 * Überprüft ob die gefragte Tabelle existiert.
	 *
	 * @param string $tableName
	 * @return boolean
	 */
	function tableExists($tableName) {
		$tables=$this->admin_get_tables();

		foreach($tables as $table) {
			if(strtoupper($tableName)==strtoupper($table)) {
				$tblExists=true;
			}
		}
		return $tblExists;
	...
	function ddl_dropTables($tableNames) {
		if($tableNames!='') {
			return $this->admin_query("DROP TABLE ".$tableNames);
		}
	}

	function ddl_dropTableFields($tableName,$fieldNames) {
		return $this->admin_query('ALTER TABLE '.$tableName.' DROP '.$fieldNames);
	}

	function ddl_addTableFields($tableName,$fields) {
		if(is_array($fields)) {
			...
			foreach($fields as $fieldName=>$field) {
				if($i!=(count($fields)-1)) {
					$query.=$this->ddl_wrapTableField($field);
				} else {
					$query.=$this->ddl_wrapTableField($field,false);
				}
				$i++;
			}
			$query.=')';
			return $this->admin_query($query);
		}
	}

	function ddl_createTable($tableName,$fields,$primaryKey=null) {
	...
	function ddl_updateTableField($tableName,$fields) {
		global $db;
		...
		return $db->admin_query($query);
	}

	function ddl_wrapTableField($field,$comma=true) {

		$fieldLen=$field["length"]!=''?'('.$field["length"].')':'';
		$autoIncrement=$field["autoIncrement"]?' auto_increment':'';
		$defaultValue=$field["default"]!=''?' '.$field["default"]:'';


		$query.=$field["name"].' '.$field["type"].$fieldLen.$defaultValue.$autoIncrement;
		if($comma) { $query.=', '; }
		//echo $this->view_array($field);
		return $query;
	}

	function ddl_addPrimaryKey($tableName,$fieldNames) {
		$query='ALTER TABLE '.$tableName.' ADD PRIMARY KEY ('.$fieldNames.')';
		return $this->admin_query($query);
	}

	function ddl_dropPrimaryKey($tableName) {
		$query='ALTER TABLE '.$tableName.' DROP PRIMARY KEY';
		return $this->admin_query($query);
	}

	function getPrimaryKey($tableName) {
		$result=$this->exec_SELECTquery("*",$tableName,'');
		$fields=$this->getFieldInfo($result);
		$primaryKeys=array();
		foreach($fields as $field) {
			if($field["primaryKey"]) {
	...
	function rowExists($tableName,$values) {
		$i=0;
		...
		$result=$this->exec_SELECTquery("*",$tableName,$where);
		if($this->sql_num_rows($result)) {
			return true;
		} else {
			return false;
		}
